...
// TODO (Firebase): replace this hardcoded list with companies fetched from Firestore
// e.g. a "electricCompanies" collection — each doc: { id, name }
const COMPANIES = [
...
export default function ElectricityBillingScreen({ navigation }) {
  ...
  const isMeterValid = meterNumber.length === METER_NUMBER_LENGTH;
  ...
  const isFormValid = company !== null && isMeterValid && isAmountValid;

  const handleMeterChange = (text) => {
    const digitsOnly = text.replace(/[^0-9]/g, '').slice(0, METER_NUMBER_LENGTH);
  ...
  const handleCheck = () => {
    ...
    // TODO (Firebase): verify meter number against Firestore/backend here,
    // then navigate on success (or show an error if invalid)
    navigation.navigate('ElectricityBillingConfirm', {
      ...
      >
        ...
        >
          ...
          <View style={styles.card}>
            ...
            >
              <Text style={company ? styles.dropdownTextFilled : styles.dropdownTextPlaceholder}>
            ...
            <Text style={styles.label}>
              Type <Text style={styles.labelBold}>meter number</Text>
            </Text>
            <TextInput
              style={styles.input}
