...
// TODO (Firebase): replace this hardcoded list with water boards fetched from Firestore
// e.g. a "waterBoards" collection — each doc: { id, name }
const WATER_BOARDS = [
  { id: '1', name: 'Lagos Water Corporation' },
  { id: '2', name: 'Abuja Water Board' },
  { id: '3', name: 'Rivers State Water Board' },
  { id: '4', name: 'Kano State Water Board' },
];
...
const WaterBillingScreen = ({ navigation }) => {
  ...
  const isBillNumberValid = billNumber.length === BILL_NUMBER_LENGTH;
  ...
  const isFormValid = waterBoard !== null && isBillNumberValid && isAmountValid;

  const handleBillNumberChange = (text) => {
    // Only allow digits, capped at 11
    const digitsOnly = text.replace(/[^0-9]/g, '').slice(0, BILL_NUMBER_LENGTH);
  ...
  const handleCheck = () => {
    ...
    // TODO (Firebase): verify bill number against Firestore/backend here,
    // then navigate on success (or show an error if invalid)
    navigation.navigate('WaterBillingConfirm', {
      waterBoard: waterBoard.name,
      billNumber,
      amount: finalAmount,
    });
  };
      ...
      >
        ...
        >
          ...
          <View style={styles.card}>
            {/* Water board dropdown */}
            {/* TODO (Firebase): WATER_BOARDS above is placeholder data —
                replace with real water boards fetched from Firestore */}
            <TouchableOpacity
            ...
            >
              <Feather
                name="droplet"
                size={18}
                color={waterBoard ? '#2563EB' : '#B3B3BD'}
                style={{ marginRight: 10 }}
              />
              <Text
                style={[
                  waterBoard ? styles.dropdownTextFilled : styles.dropdownTextPlaceholder,
                  { flex: 1 },
                ]}
              >
                {waterBoard ? waterBoard.name : 'Choose water board'}
              </Text>
            ...
            <Text style={styles.label}>
              Enter <Text style={styles.labelBold}>Bill number</Text>
            </Text>
            <TextInput
              style={styles.input}
              ...
              value={billNumber}
              onChangeText={handleBillNumberChange}
              maxLength={BILL_NUMBER_LENGTH}
            />
            ...
            <Text style={styles.helperText}>
              Please enter the correct bill number to check information.
            </Text>
          ...
          <View style={styles.modalContent}>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Feather name="droplet" size={16} color="#2563EB" style={{ marginRight: 10 }} />
